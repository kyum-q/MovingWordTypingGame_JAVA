...
public class GameFrame extends JFrame {
	...
	private void makeToolBar() {
		...
		levelBtn.addActionListener(new LevelSetAction()); // set 버튼 누를시 게임 설정 Frame 등장
	}
	
	public void startGameTh() { 
		scorePanel.scoreInit();
		gameThread = new GameThread(gamePanel, groundPanel, textTh); // gameThread 생성
	...
	private class StartAction implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			if(gamePanel.getGameStart()==0) // game이 시작되지 않았을 때
				startGameTh(); // 게임 시작
		}
	}
	private class StopAction implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			if(gamePanel.getGameStart()==1) // 게임이 시작되었을 때
				gameThread.interrupt(); // 게임 종료
		}
	}
	private class LevelSetAction implements ActionListener {
		public void actionPerformed(ActionEvent e) {
				setFrame = new GameSetFrame(textSource, gamePanel); // setFrame 생성
		}
	}
}